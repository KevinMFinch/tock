...
use crate::ieee802154::device::MacDevice;
use crate::net::ieee802154::MacAddress;
use crate::net::ipv6::ip_utils::{ip6_nh, IPAddr};
use crate::net::ipv6::ipv6::{IP6Header, IP6Packet, IPPayload, TransportHeader};
use crate::net::ipv6::ipv6_send::{IP6SendStruct, IP6Sender};
use crate::net::sixlowpan::sixlowpan_compression;
use crate::net::sixlowpan::sixlowpan_state::{Sixlowpan, SixlowpanState, TxState};
use crate::net::udp::udp::UDPHeader;
use crate::net::udp::udp_send::{UDPSendStruct, UDPSender};
...
pub struct ThreadClient<'a, A: time::Alarm> {
    alarm: A,
...
pub unsafe fn initialize_all(
    mux_mac: &'static crate::ieee802154::virtual_mac::MuxMac<'static>,
    mux_alarm: &'static MuxAlarm<'static, Alrm: time::Alarm>,
) -> &'static ThreadClient<
    'static,
    crate::virtual_alarm::VirtualMuxAlarm<'static, Alrm: time::Alarm<'static>>,
> {
    let radio_mac = static_init!(
        crate::ieee802154::virtual_mac::MacUser<'static>,
        crate::ieee802154::virtual_mac::MacUser::new(mux_mac)
    );
    mux_mac.add_user(radio_mac);
    let sixlowpan = static_init!(
        Sixlowpan<'static, Alrm: time::Alarm<'static>, sixlowpan_compression::Context>,
        Sixlowpan::new(
    ...
    let ipsender_virtual_alarm = static_init!(
        VirtualMuxAlarm<'static, Alrm: time::Alarm>,
        VirtualMuxAlarm::new(mux_alarm)
    );

    let ip6_sender = static_init!(
        IP6SendStruct<'static, VirtualMuxAlarm<'static, Alrm: time::Alarm<'static>>>,
        IP6SendStruct::new(
            ip6_dg,
    ...
    let udp_lowpan_test = static_init!(
        ThreadClient<'static, VirtualMuxAlarm<'static, Alrm: time::Alarm>>,
        ThreadClient::new(
            //sixlowpan_tx,
            //radio_mac,
            VirtualMuxAlarm::new(mux_alarm),
...
impl<'a, A: time::Alarm> crate::net::udp::udp_send::UDPSendClient for ThreadClient<'a, A> {
    fn send_done(&self, result: ReturnCode) {
        match result {
            ReturnCode::SUCCESS => {
                debug!("Packet Sent from kernel!");
...
impl<'a, A: time::Alarm> ThreadClient<'a, A> {
    pub fn new(
...
impl<'a, A: time::Alarm> time::Client for ThreadClient<'a, A> {
    fn fired(&self) {
